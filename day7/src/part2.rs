...
fn add_joker(winnings: &mut HashMap<char, i8>) -> HashMap<char, i8> {
    let mut max = ('J', 0);
    if let Some(joker) = winnings.clone().get(&'J') {
        //println!("before: {winnings:?}");
        for mut win in winnings.clone() {
            if win.1 > max.1 && win.0 != 'J' {
                max.0 = win.0;
                max.1 = win.1;
            }
        }
        if let Some(win) = winnings.get_mut(&max.0) {
            if max.0 != 'J' {
                *win += joker;
                winnings.remove(&'J');
            }
        }
        //println!("after: {winnings:?}");
    }
    winnings.clone()
}

fn get_label(mut winnings: HashMap<char, i8>) -> Label {
    winnings = add_joker(&mut winnings);
